...
function getRecipe(){
    ...
    $.ajax({
        ...
        for (i=0;i < res.hits.length;i++){
            var newRecipeItem = $("<div class='wrapper col m4 s12 card small' id='recipeCard'><div id='recipePicEdge'class='card-image waves-effect waves-block waves-light'><img id='recipePic'class='activator' src='" + res.hits[i].recipe.image + "'></div><div class='card-content'><span class='card-title activator grey-text text-darken-4'>" + res.hits[i].recipe.label + "<i class='material-icons right'>more_vert</i></span><p><a href='" + res.hits[i].recipe.url + "' target='_blank'>Click here for Recipe</a></p></div><div class='card-reveal'><span class='card-title grey-text text-darken-4'>" + res.hits[i].recipe.label + "<i class='material-icons right'>close</i></span><h6>Ingredient list:</h6><ul id='recipeIngredients" + i + "'></ul><p><a href='" + res.hits[i].recipe.url + "' target='_blank'>Click here for Recipe</a></p></div></div>");
